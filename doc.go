...
//     representations of LwDITA (XDITA,HDITA,MDITA) (and maybe throw
//     in JSON too, for compatibility with pandoc)
//   - Provide a nice one-size-fits-all representation of "documents"
//   - Correctly handle XML mixed content, which means not relying
//     too much on Golang's struct backquote-annotations for XML;
//     they are highly record-oriented and would require a lot of
//     nested "innerxml" attributes, and who the heck needs those ?
//   - Also, Golang's encoder/xml package does not have models for s
//     parse trees, ASTs, or AST nodes, so ...
...
//
// NOTE DEFINITIONS:
//   - A "tag" is everything btwn "<" and the closest ">", and does
//     include the attributes.
//   - An "elm" [element] is the start tag, end tag, and everything in-btwn.
...
// that, we rely on our own wits, and we DO try to parse and make sense of
// XML declarations (the file-initial "<?xml ..."), and we check DOCTYPE
// declarations.
//
...
// code format sensibilities (or highly aggressively reformatting editors).
//
...
//
// For tags, NS holds any XML namespaces (WITH the trailing colon !), but
// NS can also be "<?", "<!" (without that trailing colon !) and then we
// also use StringValue. This convention is also used for for CharData !
//
//   - <? = PI, sort-of including opening "<?xml"
//     (mnemonic: "?" = "IS it XML_?_")
//   - <! = Comments, CDATA, DOCTYPE (& in DTDs [only]:
//     ELEMENT, ATTLIST, ENTITY, NOTATION, Cond'l section)
//
